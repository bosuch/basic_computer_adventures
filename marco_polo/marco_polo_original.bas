...
2590 '
2600 RN=RNDC1)*FQ : IF RN<2 THEN 2610 ELSE IF RN<3.5 THEN 2630 ELSE RETURN
...
2740 PRINT "your remaining camels and jewels." : JL=0 : 8=0 : GOTO 3320
...
3080 PRINT "With shooting that good, the Khan will want you in his army." : FA=3 3090 PRINT "Your hunting yields" FA "sacks of food." : F=F+FA : RETURN
3100 '
...
3670 IF MID$CS$(RN),I,1)<>CHR$(ASC(MID$(X$,I,l))-32) THEN 3700 ELSE NEXT I
3680 S2=60*VAL(HID$<TIME$,4,2))+VAL(RIGHT$(TIME$,2)) : 'End timer
